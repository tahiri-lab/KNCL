...
"""
Branch-length scaling helper for k-NCL-based tree completion.

Workflow:
    1. Scale every Newick tree in a multi-tree input file.
    2. Run k-NCL on selected scaled trees.
    3. Unscale completed trees using the saved scaling factors.
"""
...
def split_newick_records(text: str) -> List[str]:
    """Split text into Newick records using semicolons outside quotes/comments."""
    records: List[str] = []
    buf: List[str] = []
    in_quote = False
    in_comment = False
...
def find_branch_colon(suffix: str) -> Optional[int]:
    """Return index of branch-length colon outside quotes, if present."""
    in_quote = False
    in_comment = False
    i = 0
...
def format_length(x: float) -> str:
    ...
    # 12 significant digits keeps output readable while avoiding excessive rounding.
    return f"{x:.12g}"
...
def leaf_map(root: Node) -> Dict[str, Node]:
    ...
    for leaf in leaves(root):
        name = leaf_name_from_suffix(leaf.suffix)
        if name in result:
            duplicates.add(name)
        result[name] = leaf
    ...
    if duplicates:
        raise ValueError(
            "Duplicate leaf names are not supported: " + ", ".join(sorted(duplicates))
        )
...
def make_factor_file(
    ...
    return {
        ...
        "scaled_output_file": args.output,
...
def cmd_scale(args: argparse.Namespace) -> int:
    ...
    for idx, tree in enumerate(trees, start=1):
        ...
        # Forces duplicate-name detection before pairwise computations.
        leaf_map(tree)
...
def parse_tree_indices(value: Optional[str], tree_count: int, factor_count: int) -> List[int]:
    ...
    try:
        ...
        raise ValueError("--tree-indices must be a comma-separated list of integers, e.g. 1,2,5.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Independent batch branch-length scaling for Newick trees."
    )
    sub = parser.add_subparsers(dest="command", required=True)

    p_scale = sub.add_parser(
        "scale",
        help="Scale every Newick tree in a multi-tree input file.",
    )
    p_scale.add_argument("--input", required=True, help="Input Newick file containing one or more trees.")
    p_scale.add_argument("--output", required=True, help="Output Newick file for scaled trees.")
    p_scale.add_argument("--factors", required=True, help="JSON file for scaling/unscaling factors.")
    p_scale.add_argument(
        "--method",
        choices=["median-all-pairwise", "mean-all-pairwise", "total-tree-length"],
        default="median-all-pairwise",
        help="Independent scaling method. Default: median-all-pairwise.",
    )
    p_scale.set_defaults(func=cmd_scale)

    p_unscale = sub.add_parser(
        "unscale",
        help="Apply saved inverse scale factors to completed/scaled trees.",
    )
    p_unscale.add_argument("--input", required=True, help="Input Newick file containing scaled completed tree(s).")
    p_unscale.add_argument("--output", required=True, help="Output Newick file for unscaled completed tree(s).")
    p_unscale.add_argument("--factors", required=True, help="JSON factor file produced by the scale command.")
